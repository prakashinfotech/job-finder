...
const filters = [
  ...
  {
    title: "Salary",
    options: ["Any salary", "Rs. 5,000+", "Rs. 10,000+", "Rs. 15,000+"],
  },
  {
    title: "Work mode",
...
const infoCards = [
  ...
  "Verified part-time employers",
];

const faqItems = [
  "What are Part Time Jobs?",
  "Which Companies Offer Part Time Jobs in India?",
  "Can Freshers Apply for Part Time Jobs?",
  "What is the Salary for Part Time Jobs?",
...
export default function PartTimeJobsPage() {
  const partTimeStaticJobs = useMemo(
    () => staticJobs.filter((job) => normalize(job.type) === "part time"),
    [],
  );

  const [jobs, setJobs] = useState<Job[]>(partTimeStaticJobs);
    ...
    fetch("/api/jobs", { cache: "no-store" })
      ...
      .then((payload: { jobs?: Job[] } | null) => {
        ...
        if (payload?.jobs?.length) {
          const filtered = payload.jobs.filter((job) => normalize(job.type) === "part time");
          setJobs(filtered.length >= partTimeStaticJobs.length ? filtered : partTimeStaticJobs);
        }
      })
      .catch(() => {
        if (isCurrent) setJobs(partTimeStaticJobs);
      });
  ...
  return (
    <main className="freshers-page">
      ...
      <section className="freshers-search-shell">
        <div className="container">
          <div className="freshers-search-card">
            <label htmlFor="job-search">Search part time jobs by title, company or skill</label>
        ...
        <div className="container freshers-layout">
          ...
          <div className="freshers-results">
            <div className="results-heading">
              <div>
                <p>Part Time Jobs</p>
                <h1>Part Time Jobs - {filteredJobs.length} Verified Vacancies</h1>
              </div>
              <label>
                Sort by
            ...
            <div className="job-card-list">
              {filteredJobs.length > 0 ? (
                ...
                <div className="freshers-empty-state">
                  ...
                  <p>Try changing the filters or clearing them to see more part time jobs.</p>
      ...
      <section className="freshers-info-section">
        <div className="container freshers-info-card">
          <div className="freshers-info-copy">
            <p>Know more about Part Time Jobs</p>
            <h2>Earn flexibly with verified part time jobs across India</h2>
      ...
      <section className="freshers-faq-section">
        <div className="container">
          <h2>Know more about part time jobs</h2>
          <div className="freshers-faq-list">
            {faqItems.map((item) => (
              <details key={item}>
                <summary>{item}</summary>
                <p>
                  Part time jobs offer flexible working hours ideal for students, homemakers, and anyone looking
                  to earn extra income. Roles include delivery, data entry, telecalling, tutoring, retail, and
                  content writing. Complete your profile on JobFinder and apply to verified part time jobs near you.
                </p>
              </details>
            ))}
          </div>
